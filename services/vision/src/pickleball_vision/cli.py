...
from pickleball_vision.worker.runtime import run_configured_worker
...
def build_parser() -> argparse.ArgumentParser:
    ...
    worker_parser = subparsers.add_parser(
        "worker",
        help="run the outbound-only MongoDB-leased analysis worker",
    )
    worker_parser.add_argument(
        "--pipeline-plan",
        type=Path,
        help=("trusted local JSON plan; defaults to PICKLEBALL_VISION_WORKER_PIPELINE_PLAN"),
    )
    worker_parser.add_argument(
        "--worker-id",
        help="optional stable lease owner ID; a process-specific ID is generated by default",
    )
    worker_parser.add_argument(
        "--poll-interval",
        type=float,
    ...
    worker_parser.add_argument(
        "--once",
        action="store_true",
        help="claim at most one job and exit (useful for validation and scheduled invocations)",
    )
    return parser
    ...
    try:
        ...
        if args.command == "worker":
            worker_settings = WorkerSettings.from_env(
                worker_id=cast(str | None, args.worker_id),
                pipeline_plan_path=cast(Path | None, args.pipeline_plan),
                poll_interval_seconds=cast(float | None, args.poll_interval),
            )
            ...
            _print_json(
                {
                    "mode": "once" if cast(bool, args.once) else "continuous",
                    "processed": processed,
